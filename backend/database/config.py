# This config file is meant to store your postgres password for the db.py file

# NOTE: This file should be ignored by the .gitignore file, so you can safely commit and push this file.
# However, take great precaution as this is a sensitive file that contains your Postgres password

# Double-check that this file is ignored by .gitignore before committing/pushing your password
postgres_password = ""
